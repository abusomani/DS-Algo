...
/*
    ...
    At same time path compression is making grandparent as parent of present node. ( ADD one line in find id[parent] = id[id[parent]])

    While union operation, make the smaller tree child of larger tree. ( to have less sized resultant trees)

    Advantage : Takes log*N complexity. Iterated logarithm
*/
#include <bits/stdc++.h>
using namespace std;
...
ll find(ll a)
{
    ...
    {
        id[parent] = id[id[parent]]; // JUST ONE LINE FOR PATH COMPRESSION
        parent = id[parent];
    }
    return parent;
}
